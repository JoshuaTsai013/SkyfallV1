...
[NodeDescription(name: "NavigateToPlayerCustom", story: "[Agent] navigate to [Target] when not [insight]", category: "Action/MyActions", id: "0cf6cb033b4d4fc2c91bdbe2db32b7bf")]
public partial class NavigateToPlayerCustomAction : Action
{
    ...
    protected override Status OnUpdate()
    {
        ...
        if (distance <= (DistanceThreshold + m_ColliderOffset) && Insight.Value)
        {
        ...
        if (m_NavMeshAgent != null)
        {
            ...
            if (!m_NavMeshAgent.pathPending && m_NavMeshAgent.remainingDistance <= m_NavMeshAgent.stoppingDistance && Insight.Value)
            {
                return Status.Success;
            }
        }
    ...
    private Status Initialize()
    {
        ...
        if ((GetDistanceXZ() <= (DistanceThreshold + m_ColliderOffset)) && Insight.Value)
        {
            return Status.Success;
        }
